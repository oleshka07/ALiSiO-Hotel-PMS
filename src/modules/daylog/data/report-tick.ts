...
export function runDaylogReportTickIfDue(db: any): void {
  ...
  try { reconcile = reconcileDay(date); } catch (e: any) {
    reconcileError = e?.message || String(e);
    console.log('[daylog] reconcile failed:', reconcileError);
  }

  const worthPosting = summary.count > 0
    || !!reconcileError
    || (reconcile ? reconcile.bookings.arrivalsToday > 0 || reconcile.issues.length > 0 : false);
