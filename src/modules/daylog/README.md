...
## Вечірня звірка з PMS (`data/reconcile.ts`)

Після підсумку дня звіт додає блок «🔍 Звірка з PMS» — **тільки читання**,
нічого не записує:

- **Карта** — сума карткових надходжень журналу проти `payments`
  (`method IN ('card','online')`, `status='completed'`, за цю дату). Порівнюються
  **лише надходження**: карткова покупка не має «гасити» виторг.
- **Готівка** — те саме для `method='cash'`, якщо в PMS є готівкові платежі.
- **Заїзди** — `reservations` з `check_in` = дата, без `cancelled/no_show/draft`:
  скільки оплачено (`paid`/`prepaid`), скільки без оплати, поіменний список
  боржників із сумами, і скільки записів про проживання є в журналі.

Блок «❗️ Не сходиться» людською мовою пояснює кожну розбіжність: оплату не
провели в PMS, Андрій не записав платіж, гість не оплатив, або заїзд не внесено
в журнал. Якщо все збігається — «✅ Все сходиться».

## Автоматика

`runDaylogReportTickIfDue(db)` викликається з `getDb()` — раз на добу після
`DAYLOG_REPORT_HOUR` (22:00 Прага) постить підсумок + звірку в чат. Cron не
потрібен. Звіт виходить навіть у «тихий» день, якщо є заїзди чи розбіжності.
